...
\begin{itemize}
    \item $k_1=1.2$, $b=0.75$ (BM25)
    \item $w_{title}=2.0,\  w_{text}=1.0$ (BM25F)
    \item $\alpha=1.0,\, \beta=0.3,\, K=10$ (PRF)
\end{itemize}
\end{block}
...
\toprule
Phương pháp & P@1 & Recall@10 & MAP \\
\midrule
BM25 Baseline & 0.5600 & 0.8202 & 0.6480 \\
BM25F & 0.5633 & 0.8202 & 0.6524 \\
BM25 + PRF & 0.5133 & 0.8203 & 0.6139 \\
\bottomrule
\end{tabular}
\end{table}
...
\vspace{.6em}
\begin{itemize}
    \item \textbf{BM25F} cải thiện chỉ số MAP so với BM25, nhờ tận dụng thông tin từ title.
    \item \textbf{PRF} giúp tăng Recall nhưng lại làm giảm Precision@1 và MAP, có thể do việc thêm các term không liên quan khiến truy vấn bị lệch chủ đề.
\end{itemize}
\end{frame}
...
\begin{block}{Tóm tắt}
\begin{itemize}
    \item \textbf{BM25F} đạt kết quả tốt nhất, cho thấy việc tận dụng thông tin từ nhiều field giúp cải thiện hiệu quả truy xuất.
    \item \textbf{PRF} giúp tăng Recall nhưng có thể làm giảm Precision và MAP nếu không kiểm soát tốt việc mở rộng truy vấn; do đó, cần áp dụng kỹ thuật lọc term phù hợp để tránh truy vấn bị lệch chủ đề.
\end{itemize}
\end{block}
\vspace{1em}
