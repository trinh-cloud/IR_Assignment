...
- **Giai đoạn 2: Lớp Data & Indexing:** Quản lý bởi `Indexer` và dataset loader (tải trực tiếp từ HuggingFace BEIR). Đặc biệt triển khai Inverted Index cho văn bản trơn và Inverted Index đa phân vùng (Multi-field: Title, Abstract) để lưu trữ `term_frequency` và `doc_length` độc lập theo từng vùng. 
